...
interface Rec {
  ...
  p1CandidateCount: number; p2CandidateCount: number; p2FormulaCount: number;
  selectedFormulaName: string; selectedAuthority: string;
  selectedSourceCaseRef: string; selectedVisitRef: string; selectedSourceEvidenceRef: string;
  selectedCompositionPresent: boolean;
...
async function runOne(id: string, input: string): Promise<Rec> {
  try {
    ...
    const p2Formula = cands.filter((c) => c.sourceAuthority === 'P2_CASE_DERIVED' && typeof c.visitRef === 'string' && c.visitRef.length > 0).length;
    const selectedRef = workspace.clinicalDecisionSpine.formulaSelection?.selectedCandidateRef;
    const selectedCand = selectedRef ? cands.find((c) => c.id === selectedRef) : undefined;
    return {
      id,
      ...
      selectedSourceCaseRef: selectedCand?.sourceCaseRef ?? '',
      selectedVisitRef: selectedCand?.visitRef ?? '',
      selectedSourceEvidenceRef: selectedCand?.sourceEvidenceRef ?? '',
      selectedCompositionPresent: Array.isArray(formulaField?.composition) && (formulaField.composition as unknown[]).length > 0,
    ...
    return { id, error: e instanceof Error ? e.message : String(e), diseaseName: '', primaryPattern: '', treatmentPrinciple: '', p1CandidateCount: 0, p2CandidateCount: 0, p2FormulaCount: 0, selectedFormulaName: '', selectedAuthority: '', selectedSourceCaseRef: '', selectedVisitRef: '', selectedSourceEvidenceRef: '', selectedCompositionPresent: false, successfulSubmit: false, forcedFinalization: false, stepCount: 0, toolCalls: 0, tokens: 0, latencyMs: 0 };
  }
}

await loadIndex();

const OUT_FILE = `reports/h15.2.7-targeted-${modelId.replace(/[^a-zA-Z0-9._-]/g, '_')}.jsonl`;
...
for (const r of results) {
  console.log(`${r.id}: ${r.error ? 'ERR' : r.forcedFinalization ? 'FORCED' : 'OK'} primary=${r.primaryPattern.slice(0, 16) || '-'} ` +
    `P1cand=${r.p1CandidateCount} P2cand=${r.p2CandidateCount} P2formula=${r.p2FormulaCount} ` +
    `sel=${r.selectedFormulaName || '-'} auth=${r.selectedAuthority || '-'} ` +
    `case=${r.selectedSourceCaseRef || '-'} visit=${r.selectedVisitRef || '-'} comp=${r.selectedCompositionPresent} ` +
    `steps=${r.stepCount} tokens=${r.tokens}${r.error ? ' ERR=' + r.error.slice(0, 60) : ''}`);
}
